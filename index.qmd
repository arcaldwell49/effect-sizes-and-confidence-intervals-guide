...
$$ $$
Z_\text{Lower} = Z_V - 1.96\times SE_{Z_V}
$$

$$
Z_\text{Upper} = Z_V + 1.96\times SE_{Z_V}
...
Since the sampling distribution of $h$ is symmetric, we can calculate the confidence intervals as described previously in @eq-ci-low and @eq-ci-high. To calculate Cohen's $h$, we can use the `cohens_h` function in the `effectsize` package in R.
...
LOR_{UP} = LOR + 1.96\times SE_{LOR}
$$ 
Then the final step we can convert these back to $OR$ confidence intervals by taking the exponential transformation of them: $OR_{UP}=\exp(LOR_{UP})$ and $OR_{UP}=\exp(LOR_{UP})$.
...
Where $n_C$ and $n_T$ are the total sample sizes *within* the control and treatment group, respectively. The standard error can then be used to compute the 95% confidence intervals using the formulas described in @eq-ci-high and @eq-ci-low

$$
RD_{LO} = RD - 1.96 \times SE_{RD}
$$

$$
RD_{UP} = RD + 1.96 \times SE_{RD}
$$
