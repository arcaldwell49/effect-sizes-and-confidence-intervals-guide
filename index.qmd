...
CF <- exp ( lgamma(df/2) - log(sqrt(df/2)) - lgamma((df-1)/2) )
...
## Calculating $d$ Values from Test-Statistics

When effect sizes are unreported, but test-statistics are available, we can calculate the standardized mean difference ($d$) from t-statistics (or p-values) and the sample size. For a between-groups Cohen's d with a pooled standard deviation denominator ()
